...
local function create_edit_buffer(win_cmd, mdbufnr, row, col, srow, scol, erow, ecol, filetype)
    ...
    local bufname = vim.api.nvim_buf_get_name(mdbufnr) .. ':' .. srow .. '-' .. erow
...
local edit_code_block = function(opts)
    local win_cmd
...
local edit_code_block_org = function(opts)
    local win_cmd
    if opts and opts.args and opts.args ~= '' then
        win_cmd = opts.args
    else
    ...
    if name then
        ...
        if btype == 'SRC' and parameter then
            ...
            local language = string.match(table.concat(vim.api.nvim_buf_get_text(mdbufnr, srow, scol, erow, ecol, {})),
                '%S*')
            if contents then
...
local edit_code_block_selection = function(opts)
...
local create_commands = function()
    vim.api.nvim_create_user_command('EditCodeBlock',
        function(opts) edit_code_block(opts) end,
        {
            nargs = '*',
    ...
    vim.api.nvim_create_user_command('EditCodeBlockOrg',
        function(opts) edit_code_block_org(opts) end,
        {
            nargs = '*',
            complete = function(_, _, _)
                return { "split", "vsplit", "tabnew", "rightbelow", "leftabove" }
    ...
    vim.api.nvim_create_user_command('EditCodeBlockSelection',
        function(opts) edit_code_block_selection(opts) end,
        {
            nargs = '+',
            complete = function(_, _, _)
                return { "split", "vsplit", "tabnew", "rightbelow", "leftabove" }
